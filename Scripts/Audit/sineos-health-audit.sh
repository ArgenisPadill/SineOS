...
# Estado: pendiente de revalidación v1.2.0
...
if [[ "$local_n" -eq 0 ]]; then
  ...
  info "$local_n paquete(s) aparecen como instalados localmente o sin versión disponible en las fuentes actuales; se inventarían, no se eliminan automáticamente."
...
if has debsecan; then
  ...
  vuln_n="$(printf '%s\n' "$vuln" | sed '/^$/d' | wc -l)"
  info "Inventario general debsecan: $vuln_n entrada(s). El control accionable es --only-fixed; el total puede incluir vulnerabilidades sin corrección disponible o no accionables."
else
...
if has podman; then
  ...
  moving="$(podman ps -a --format '{{.Names}}|{{.Image}}' | grep -E ':(main|latest)else
...
obsolete="$(apt list '~o' 2>/dev/null | sed '1d' || true)"
obs_n="$(printf '%s\n' "$obsolete" | sed '/^$/d' | wc -l)"
[[ "$obs_n" -eq 0 ]] && ok "Sin paquetes obsoletos/sin fuente." || { warn "$obs_n paquete(s) obsoleto(s)/sin fuente."; printf '%s\n' "$obsolete" | head -n 60; }
...
if has debsecan; then
  ...
  vuln="$(debsecan --suite trixie --format simple 2>/dev/null || true)"
  vuln_n="$(printf '%s\n' "$vuln" | sed '/^$/d' | wc -l)"
  echo "Entradas de vulnerabilidad debsecan: $vuln_n"
  [[ "$vuln_n" -eq 0 ]] && ok "Sin vulnerabilidades reportadas por debsecan." || warn "Hay vulnerabilidades registradas; revisar severidad/fix."
else
  err "debsecan no instalado: revisión de vulnerabilidades no certificable."
fi
...
if has smartctl; then
  if sudo -n true >/dev/null 2>&1; then
    while read -r dev rest; do
      [[ -n "$dev" ]] || continue
      health="$(sudo -n smartctl -H "$dev" 2>/dev/null || true)"
      echo "$health"
      echo "$health" | grep -Eqi 'PASSED|OK' && ok "SMART saludable: $dev" || warn "SMART no concluyente: $dev"
    done < <(sudo -n smartctl --scan-open 2>/dev/null || true)
  else
    warn "smartctl disponible pero sin sudo no interactivo."
  fi
...
if has podman; then
  ...
  [[ -z "$moving" ]] && ok "Sin tags móviles main/latest." || { warn "Tags móviles detectados:"; echo "$moving"; }
else
  err "Podman no disponible."
fi
...
if [[ "$ERR" -gt 0 ]]; then
 ...
 || true)"
  ...
  if [[ -n "$known_moving" ]]; then
    info "Open WebUI usa :main; riesgo conocido y registrado como TD-006."
  fi
