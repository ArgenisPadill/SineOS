...
# Estado: pendiente de primera validación
...
journal_errors="$(journalctl -p err -b --no-pager 2>/dev/null | grep -v '^-- No entries --$' || true)"
[[ -z "$journal_errors" ]] && ok "Sin errores err en el boot actual." || { warn "Journal con errores; revisar."; echo "$journal_errors" | tail -n 40; }
